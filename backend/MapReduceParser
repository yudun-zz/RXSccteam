...
public class MRTextParser {
    ...
    public static class ParserMap extends Mapper<LongWritable, Text, Text,Text> {
    	...
    	public void map(LongWritable key, Text value, Context context) 
    		throws IOException, InterruptedException, NumberFormatException, JSONException {
			...
			/*Deal with one json record*/
			  ...
			  try{
				...
				String SEP = "";
				String writeRecord = time+SEP+tweetid+SEP
								+userid+SEP+score+SEP+resultText+"\n"
				
				OutKey.set(writeRecord);
	 	...
	 	private static int getScore(String rawText){
			...
			while (iter.hasNext()) {
			    ...
			    Pattern pattern = Pattern.compile("[^A-Za-z0-9]"+key+"[^A-Za-z0-9]");
			    //System.out.println(pattern.toString());
			    Matcher  matcher = pattern.matcher(rawText.toLowerCase());
			    while(matcher.find()){
    ...
    public static class ParserReduce extends Reducer<Text, Text, Text, Text> {
